...
export class PrivateKey {
	...
	constructor(x509Binary: string) {
		...
		// @ts-ignore
		const isEncrypted = asn1['validate'](this.asn1Object, this.encryptedPrivateKeyValidator, null, errors)
		if (!isEncrypted) {
	...
	rsaDecrypt(encryptedText: string, passwordKey: string) {
		const privateKeyInfo = pki.decryptPrivateKeyInfo(this.asn1Object, passwordKey)
