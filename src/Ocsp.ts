...
export class Ocsp {
  private readonly issuerCertificate: x509Certificate;
  private readonly subjectCertificate: x509Certificate;
  private readonly ocspCertificate: x509Certificate;
  private readonly urlService: string;
  constructor(
    urlService: string,
    issuerCertificate: x509Certificate,
    subjectCertificate: x509Certificate,
    ocspCertificate: x509Certificate
  ) {
    ...
    if (!regexUrl.test(urlService)) {
      throw new ERROR_GENERAL_ERROR(
        "Revisar la url del servicio OCSP, el formato no es de URL"
      );
    }
  ...
  private getOCSPRequest(): util.ByteBuffer {
    ...
    const issuerNameHash = Buffer.from(hashIssuerNameBinary, "hex").toString(
      "binary"
    );

    const publicKeyFromASN1 = this.getASN1PublicKeyBinary();
    const hashPublicKeyFromASN1 = GlobalMethods.hash(publicKeyFromASN1, "sha1");
    const issuerKeyHash = Buffer.from(hashPublicKeyFromASN1, "hex").toString(
      "binary"
    );

    const serialNumber = Buffer.from(
      this.subjectCertificate.serialNumber,
      "hex"
    ).toString("binary");

    const asn1OCSPRequest = asn1.create(
            ...
            asn1.create(asn1.Class.UNIVERSAL, asn1.Type.SEQUENCE, true, [
              asn1.create(asn1.Class.UNIVERSAL, asn1.Type.SEQUENCE, true, [
                ...
                asn1.create(
                  ...
                  false,
                  Buffer.from(
                    "041064bb982b0f6236984ec9d8c4997b6996",
                    "hex"
                  ).toString("binary")
                ),
              ]),
  ...
  async callToService(body: Buffer): Promise<Blob> {
    ...
    if (response.status != 200) {
      throw new ERROR_GENERAL_ERROR(
        "Error al consultar el servicio " + this.urlService
      );
    }
  ...
  private dateFromASN1Date(date: string): Date {
    if (date.indexOf("Z") == -1) {
      throw new ERROR_GENERAL_ERROR(
        "Formato de fecha incorrecto, se espera YYYYMMDDHHMMSSZ"
      );
    }
    return new Date(
      date.slice(0, 4) +
  ...
  verifyOcspResponseSignature(asn1GoodOCSPBasic: asn1.Asn1): boolean {
    try {
      ...
      const tbsRespobseDataAsn1 = asn1.create(
        asn1.Class.UNIVERSAL,
        asn1.Type.SEQUENCE,
        true,
        asn1Value
      ...
      const verifiedSignature = this.ocspCertificate.rsaVerifySignature(
        tbsRespobseData,
        signature,
        "sha1"
      );
  ...
  verifyOcspResponse(asn1OcspResponse: asn1.Asn1): ocspResponseVerify {
    //@ts-ignore
    const ocspResponseStatus = Buffer.from(
      (asn1OcspResponse.value[0] as Asn1Efirma)["value"]
    ).toString("hex");
  ...
  verifyCertificateStatus(asn1OCSPBasic: asn1.Asn1): certificateStatusVerify {
    const certificateStatus = (asn1OCSPBasic.value[0] as Asn1Efirma)["value"][2]
      .value[0].value[1];
    if (certificateStatus.type === OCSP_CERTIFICATE_STATUS.GOOD) {
      return { status: "GOOD" };
    } else if (certificateStatus.type === OCSP_CERTIFICATE_STATUS.REVOKED) {
      const revocationTime = this.dateFromASN1Date(
        certificateStatus.value[0].value
      );
  ...
  async verify(): Promise<verifyResponse> {
    ...
    if (ocspResponseStatus.status === OCSP_REQUEST_STATUS.SUCCESSFUL) {
      //@ts-ignore
      const asn1OCSPBasic = asn1.fromDer(
        (asn1OcspResponse.value[1] as Asn1Efirma)["value"][0].value[1].value
      );
      ...
      if (verify) {
        ...
        throw new ERROR_GENERAL_ERROR(
          "La firma de la respuesta OCSP no corresponde"
        );
      }
    } else {
      throw new ERROR_GENERAL_ERROR(
        "No fue posible realizar la validación OCSP \n" + ocspResponseStatus
      );
    }
  }
  ...
  private getASN1PublicKeyBinary(): string {
    ...
    //@ts-ignore
    const publicKeyAsn1Node =
      (asn1IssuerCert.value[0] as Asn1Efirma).value[6].value[1].bitStringContents;
    const ocspIssuerPublicKey = publicKeyAsn1Node.slice(
      1,
      publicKeyAsn1Node.length
    );
