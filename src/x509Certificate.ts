...
export class x509Certificate {
  ...
  public static getSubjectField(
    ...
    if (subject && subject.attributes && Array.isArray(subject.attributes)) {
      //@ts-ignore
      const findResult = subject.attributes.find(
        (field) => field[type as keyof typeof field] === valueToFind,
      );
      if (findResult.value) {
    ...
    if (!value) {
      throw new ERROR_GENERAL_ERROR(
        `${valueToFind} no encontrado en el tipo ${type}`,
      );
    }
    return value;
  }
  ...
  private certificateFromAsn1(asn1Object: asn1.Asn1) {
    try {
      ...
      throw new ERROR_GENERAL_ERROR(
        "Verifique el archivo, no fue posible convertir el ASN1 a certificado",
      );
    }
  }
  ...
  getCertiticateType() {
    ...
    for (let i in extensions) {
      if (
        extensions[i].name === "extKeyUsage" &&
        extensions[i].emailProtection === true &&
        extensions[i].clientAuth === true
      ) {
        return "EFIRMA";
      }
      if (
        extensions[i].name === "keyUsage" &&
        extensions[i].digitalSignature === true &&
        extensions[i].nonRepudiation === true &&
        extensions[i].dataEncipherment === false &&
        extensions[i].keyAgreement === false
      ) {
        return "CSD";
      }
    }
  ...
  getSubjectType() {
    try {
      const subjectRfc = x509Certificate.getSubjectField(
    ...
    try {
      ...
      if (err instanceof Error) {
        if (
          err.message
            .toString()
            .indexOf("The parent certificate did not issue") >= 0
        ) {
          throw new ERROR_GENERAL_ERROR(
            "El certificado recibido no fue emitido por el emisor, verifique que el emisor sea el correcto y que el certificado no este alterado",
          );
        }
      }
      throw err;
  ...
  rsaVerifySignature(
    ...
    // @ts-ignore
    const verified = this.certificate.publicKey["verify"](
      messageDigest,
      signature,
    );
    return verified;
